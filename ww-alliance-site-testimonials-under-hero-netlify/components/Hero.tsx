    ...
    <section className="relative overflow-hidden">
      ...
      <div className="container-safe grid gap-10 py-16 lg:py-24 place-items-center text-center">
        <motion.div variants={stagger} initial="hidden" animate="show">
          ...
          <motion.div
            variants={fadeUp}
            className="flex items-center justify-center gap-1 text-[color:var(--brand)] mb-3"
            aria-label="Rated 5 out of 5 stars"
          >
            {Array.from({ length: 5 }).map((_, i) => (
              <Star key={i} size={18} className="drop-shadow" fill="currentColor" />
            ))}
          </motion.div>

          <motion.p
            variants={fadeUp}
            className="uppercase text-sm tracking-widest text-[color:var(--brand)]"
          >
          ...
          <motion.div variants={fadeUp} className="mt-6 flex gap-3 justify-center">
