...
/** Fixed gold starfield overlay (auto-tunes, cleans up on unmount). */
export default function GoldStarsOverlay() {
  useEffect(() => {
    ...
    const SPEED_MIN = 0.05,
      SPEED_MAX = 0.55;
    let TWINKLE_AMPL = 0.6;
    const DRIFT_FREQ = 0.001;
    ...
    const layers = [
      ...
      { stars: [] as any[], speedScale: 1.0, densityBase: DENSITY_FG, spritePicker: null as any },
    ];
    ...
    function makeGlow(size: number) {
      ...
      g.addColorStop(0.0, "rgba(255,242,170,0.98)");
      g.addColorStop(0.3, "rgba(255,223,77,0.95)");
      ...
      c.beginPath();
      c.arc(0, 0, Math.max(0.5, size * 0.3), 0, Math.PI * 2);
      c.fill();
      return off;
    }
    ...
    const SPR_MED = makeGlow(1.4);
    const SPR_LARGE = makeGlow(1.8);
    ...
    function makeStar(L: any, x: number, y: number) {
      ...
      return {
        x,
        y,
        vx: Math.cos(dir) * base,
        vy: Math.sin(dir) * base,
        r,
    ...
    function tick() {
      ...
      for (const L of layers) {
        ...
        for (let i = 0; i < sarr.length; i++) {
          ...
          const nx = s.vx + -s.vy * 0.1 * drift;
          const ny = s.vy + s.vx * 0.1 * drift;
          s.x += nx * (dt * 0.06);
          s.y += ny * (dt * 0.06);
          if (s.x < 0) s.x += width;
          ...
          const w = s.sprite.width * s.scale,
            h = s.sprite.height * s.scale;
    ...
    (function autoTune() {
      ...
      function sample() {
        ...
        if (t - tPrev >= 2500) {
          ...
          if (fps < 40) {
            MAX_STARS_CAP = Math.floor(MAX_STARS_CAP * 0.65);
            DPR_CAP = Math.max(1.0, DPR_CAP - 0.25);
            TWINKLE_AMPL = Math.min(0.55, TWINKLE_AMPL);
            resize();
          } else if (fps < 50) {
            MAX_STARS_CAP = Math.floor(MAX_STARS_CAP * 0.8);
            DPR_CAP = Math.max(1.1, DPR_CAP - 0.15);
            resize();
          }
        } else {
